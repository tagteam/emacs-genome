...
;; Last Modified: <2012-10-11 Thu 8:05 am>
...
(defcustom mediawiki-make-api-url-ext "php"
  "Extension for constructing the mediawiki-make-api-url."
  :type 'string
  :group 'mediawiki)

(defcustom mediawiki-save-ask-for-summary t
  "If nil save pages with blank summary, otherwise prompt for summary."
  :type 'boolean
  :group 'mediawiki)

(defcustom mediawiki-open-prefer-existing-buffer t
  "If non-nil and there is a buffer visiting the page, return that one.

If set to nil opening a page will always open the version of the page
from the mediawiki engine. This may overwrite unsaved edits of the
 current session."
  :type 'boolean
  :group 'mediawiki)

(defcustom mediawiki-buffer-pop-dont-pop nil
"If set to non-nil mediawiki-pop-to-buffer will switch-to rather
than pop-to buffer."
  :type 'boolean
  :group 'mediawiki)
...
(defun mediawiki-make-api-url (&optional sitename)
  (format (concat (mediawiki-site-url (or sitename mediawiki-site))
                  "api." mediawiki-make-api-url-ext)))
...
If a buffer exists visiting this page, return that one."
  (interactive
  ...
  (if (and mediawiki-open-prefer-existing-buffer
    ;; check if there is a buffer for this page (to not loose unsaved edits)
	   (get-buffer (concat mediawiki-site ": " name)))
	(switch-to-buffer  (concat mediawiki-site ": " name))
...
(defun mediawiki-get-namespaces (sitename limit)
  "Query mediawiki-site SITENAME for the first LIMIT namespaces."
  (let ((namespaces (mediawiki-api-call
		     sitename "query" 
		     (list (cons "meta" "siteinfo")
			   (cons "siprop" "namespaces|namespacealiases")
			   (cons "aplimit" (mediawiki-api-param (or limit "100"))))))
	ns)
    (delq nil (mapcar (lambda (x) (when (and (listp x) x)
				    (cons (or (caddr x) "main") (cdr (assq 'id (cadr x))))))
		      (caddr namespaces)))))

(defun mediawiki-get-page-list (sitename namespace limit)
  "Query mediawiki-site SITENAME for the first LIMIT pages in NAMESPACE."
  (let ((pages (mediawiki-api-call
		sitename "query" 
		(list (cons "list" "allpages")
		      (when namespace
			(cons "apnamespace" (mediawiki-api-param namespace)))
		      (cons "aplimit" (mediawiki-api-param (or limit "5"))))))
	page)
    (delq nil (mapcar (lambda (x)
			(when (and (listp x) x)
			  (let ((el (cdr (assq 'title (cadr x)))))
			    (cons (cadr (split-string el ":"))
				  el))))
	  (caddr pages)))))

(defun mediawiki-find-page (&optional limit)
  "Interactively select a namespace and then select
a page with completion from the list of pages in the namespace.
If LIMIT is an integer restrict the completion list. Default is 1000."
  (interactive "p")        
  (let* ((namespaces (mediawiki-get-namespaces mediawiki-site 1000))
	 (ns (cdr (assoc (completing-read "Find in namespace: " namespaces) namespaces)))
	 (page-list (mediawiki-get-page-list
		     mediawiki-site ns (if (> limit 1) limit  1000)))
	 (page (completing-read "Page name: " page-list)))
    (mediawiki-open (cdr (assoc page page-list)))))

(defun mediawiki-insert-page-list (&optional limit)
  "Interactively select a namespace and write the names of the pages
in the namespace to the current buffer. 
If LIMIT is an integer restrict the page list. Default is 1000."
  (interactive "p")        
  (let* ((namespaces (mediawiki-get-namespaces mediawiki-site 1000))
	 (ns (cdr (assoc (completing-read "Find in namespace: " namespaces) namespaces)))
	 (pages (mediawiki-get-page-list mediawiki-site ns 1000)))
    (while pages
      (insert (caar pages) "\n")
...
(defun mediawiki-pop-to-buffer (bufname &optional dont-pop)
  "If dont-pop is non-nil pop-to-buffer, else switch-to-buffer BUFNAME,
 Then execute a hook."
  (if (or dont-pop mediawiki-buffer-pop-dont-pop)
...
(defun mediawiki-save (&optional summary)
  ...
  (let ((summary 
	 (or summary
	     (when mediawiki-save-ask-for-summary (read-string "Summary: "))
	     "")))
...
(defun mediawiki-open-page-at-point-other-window ()
  "Open a new buffer with the page at point in a different window also
if `mediawiki-buffer-pop-dont-pop' is set."
  (interactive)
  (let ((mediawiki-buffer-pop-dont-pop nil))
    (mediawiki-open (mediawiki-page-at-point))))

(defun mediawiki-page-at-point ()
  "Return the page name under point.  Typically, this means
anything enclosed in [[PAGE]] or {{PAGE}}."
  (let ((pos (point))
        (eol (point-at-eol))
        (bol (point-at-bol)))
    (save-excursion
      (let* ((start  (when (or (search-backward "[[" bol t) (search-backward "{{" bol t))
                       (+ (point) 2)))
             (end    (when (or (search-forward "]]" eol t)  (search-forward "}}" eol t))
                       (- (point) 2)))
             (middle (progn
                       (goto-char start)
