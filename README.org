...
To use (or test) the emacs-genome you need to download. The
recommended way to do this is to use git. Never heard of git? It is
the stupid content tracker, a revision control system.

, apply the following command in a terminal
to clone the emacs-genome into your home:
