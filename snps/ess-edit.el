...
;;; code
;;{{{ variables
(defvar ess-edit-mark-list
  (list "line" "paragraph" "call" "buffer" "word"))
...
(defun ess-edit-mark-code ()
  "Mark code in the following order: current line,
current paragraph, call to function, current line, etc"
  (interactive)
  (when (eq last-command 'ess-edit-mark-code)
    (ess-edit-cycle-mark-list))
  (cond ((string= (car ess-edit-mark-list) "line")
	 (ess-edit-mark-line nil))
	((string= (car ess-edit-mark-list) "paragraph")
	 (mark-paragraph))
	((string= (car ess-edit-mark-list) "call")
	 (condition-case nil
	     (ess-edit-mark-call)
	   (error (progn
		    (message "curser not in call to R function")
		    nil))))
	;; ((string= (car ess-edit-mark-list) "buffer")
	 ;; (mark-whole-buffer))
	((string= (car ess-edit-mark-list) "word")
	 (mark-word))))
...
(defun ess-edit-vector2formula ()
  (interactive)
    (let* ((fname (ess-edit-read-call 1 'move))
	  ...
	  (message "Current cursor position is not on a vector")
...
(defun ess-edit-display-help-on-arg (&optional function)
  ...
  (let* ((arg (ess-edit-arg-at-point))
	...
	(message (format "%s is not an argument of %s!" arg fname))
      (setq arg arg-args)
...
(defun ess-edit-cycle-mark-list ()
  (let ((cur (car ess-edit-mark-list))
	(rest (cdr ess-edit-mark-list)))
    (setq ess-edit-mark-list rest)
    (add-to-list 'ess-edit-mark-list cur 'append)))
